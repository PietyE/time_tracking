  ...
  return (
    <>
      <div className="text-left d-flex">
        ...
        {!isFetchRateList && errorFetchRatesList && (
          <p className="rate_item m-0 py-3 p-3 text-danger">
            An error occurred while fetching rates list
          </p>
        )}
        {!isFetchRateList && !errorFetchRatesList && !showContent && (
          <p className="rate_item m-0 py-3 p-3">No data to display</p>
        )}
        {!isFetchRateList &&
          !errorFetchRatesList &&
          showContent &&
          rateList.map((item) => {
            ...
            return (
              <p key={currencyId} className="text-left rate_item m-0 p-3">
                1 {name} - {rateToRender};
              </p>
            )
          })}
