...
function User(props) {
  ...
  const userProjects = useMemo(() => {
    return projects.filter(
      (project) =>
        ...
        (project.is_active || (!project.is_active && project.total_minutes))
    )
  }, [projects, userId])
  const projectList = useMemo(() => {
    const activeProject = [...userProjects].filter(
    ...
    () =>
      userProjects.map(
        ({
          ...
          totalHours: is_full_time
            ? 'Salary'
            : parseMinToHoursAndMin(overtime_minutes, true) || 0,
          totalOvertime: overtime_minutes
            ? parseMinToHoursAndMin(overtime_minutes, true)
            : 0,
          id: id,
  ...
  useEffect(() => {
    const sortFormattedProjects = [...formattedProjects].sort(
      sortUserProjectReport
    )
    const currentProjects = sortFormattedProjects.filter(
      (project) => project.active_project
    )
    const pastProjects = sortFormattedProjects.filter(
      (project) => project.active_project === false
    )
  ...
  return (
    <div
      className={`user_info ${
        contextType.selectedUserId === userId ? 'selected' : ''
      }`}
      onClick={chooseUser}
    >
      <div className="user_info_row">
        ...
        <div className="projects">
          <span className={`${userProjects.length ? '' : 'empty'}`}>
            {projectList}
          </span>
        </div>
      ...
      <div
        className={`project_by_current_user ${
          contextType.selectedUserId === userId ? 'selected' : ''
        }`}
      >
