...
import { getUsersSelector, getAllProjectsSelector } from '../../../reducers/projects-management'
...
function CreateProjectModal({ onClose, show }) {
  ...
  const projects = useSelector(getAllProjectsSelector,isEqual)
    ...
    const projectManagers = useCallback(users.filter(user => user.role === 4),[users])
        ...
        >
          {({ values, setFieldValue }) => (
            <Form className = "pm_create_modal_form">
              ...
              <ul>
                {values.team.map(el => <TeamMemberItem key={el.user_id} data={el} setFieldValue={setFieldValue} values={values}/>)}
              </ul>
              }
