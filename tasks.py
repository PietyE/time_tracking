...
def install_dokku(ctx, env, client):
    ...
    if not is_dokku_installed:
        ...
        stdin, stdout, stderr = client.exec_command('wget https://raw.githubusercontent.com/dokku/dokku/v0.19.13/bootstrap.sh')
        display_message(stdout)
        display_message(stderr)
...
def add_key_to_dokku(ctx, env):
    ...
    ctx.run(f"cat ~/.ssh/id_rsa.pub | ssh -i {config['ssh_key_path']} "
            f"-o 'IdentitiesOnly yes' "
            f"{config['host_user']}@{config['ip']} sudo dokku ssh-keys:add $USER", pty=True)
...
def install_postgres_plugin(ctx, env, client):
    ...
    if not is_postgres_installed:
        ...
        stdin, stdout, stderr = client.exec_command('sudo dokku plugin:install '
...
def create_postgres_service(ctx, env, client):
    ...
    if not (service_name and postgres_image and postgres_image_version):
        print('"service_name", "postgres_image" and "postgres_image_version" should be set in config')
        return
...
def link_postgres_service(ctx, env, client):
    ...
    if is_service_linked:
        ...
        stdin, stdout, stderr = client.exec_command(f'dokku postgres:link '
                                                    f'{service_name} '
                                                    f'{project_name}')
        display_message(stderr)
        display_message(stdout)
...
def link_redis_service(ctx, env, client):
    ...
    if is_service_linked:
        ...
        stdin, stdout, stderr = client.exec_command(f'dokku redis:link '
                                                    f'{service_name} '
                                                    f'{project_name}')
        display_message(stderr)
        display_message(stdout)
...
def install_logspout(ctx, env, client):
    ...
    if not is_logspout_installed:
        ...
        stdin, stdout, stderr = client.exec_command('sudo dokku plugin:install '
                                                    'https://github.com/michaelshobbs/dokku-logspout.git')
        display_message(stdout)
        display_message(stderr)
...
def add_logspout_server(ctx, env, client):
    ...
    stdin, stdout, stderr = client.exec_command(f'sudo dokku logspout:server syslog+tls://{logspout_server}')
    display_message(stdout)
    display_message(stderr)
...
def add_remote_branch(ctx, env):
    ...
    if not any(map(lambda x: remote_branch_name in x and remote_address in x, remotes.stdout.splitlines())):
...
def install_letsencrypt_plugin(ctx, env, client):
    ...
    is_letsencrypt_email_setup = any([letsencrypt_email in line for line in stdout if 'DOKKU_LETSENCRYPT_EMAIL' in line])
    ...
    display_message(stderr)
    installed_domains = [line.split(':')[1].strip() for line in stdout if 'Domains app vhosts' in line]

    if domain not in installed_domains:
        print('Add required domain: {domain}...')
        stdin, stdout, stderr = client.exec_command(f'sudo dokku domains:add {project_name} {domain}')
        display_message(stderr)
        display_message(stdout)
    ...
    for domain_to_remove in domains_to_remove:
        ...
        stdin, stdout, stderr = client.exec_command(f'sudo dokku domains:remove {project_name} {domain_to_remove}')
        display_message(stderr)
        display_message(stdout)
    ...
    if not is_letsencrypt_installed:
        ...
        stdin, stdout, stderr = client.exec_command(f'cat /home/dokku/{project_name}/nginx.conf.d/letsencrypt.conf')
        ...
        if need_create_file:
            ...
            stdin, stdout, stderr = (
                client.exec_command(f'sudo chmod 777 /home/dokku/{project_name}/nginx.conf.d/letsencrypt.conf')
            )
            display_message(stderr)
            display_message(stdout)
...
def create_db_dump(ctx, env):
    ...
    config = CONFIG[env]
    if not all((config.get('local_branch_name'), config.get('postgres_service_name'), config.get('ip'))):
    ...
    export_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), project_backup_folder, subfolder)
    ...
    ctx.run(f'ssh -tt dokku@{config["ip"]} postgres:export {config["postgres_service_name"]} > "{export_path}"')
